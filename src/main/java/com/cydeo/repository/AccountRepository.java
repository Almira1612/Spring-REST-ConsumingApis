...
public interface AccountRepository extends JpaRepository<Account, Long> {
    ...
    @Query(value = "SELECT * FROM accont_details WHERE age < ?1", nativeQuery = true)
    ...
    @Query(value = "SELECT * FROM account_details WHERE name ILIKE concat('%', ?1, '%')" +
            "OR country name ILIKE concat('%', ?1, '%')" +
            "OR address name ILIKE concat('%', ?1, '%')" +
            "OR state name ILIKE concat('%', ?1, '%')" +
            "OR city name ILIKE concat('%', ?1, '%')", nativeQuery = true)
